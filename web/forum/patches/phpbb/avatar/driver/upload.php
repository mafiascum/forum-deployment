...
{
	// Hard caps applied before phpBB validation. Anything below these
	// gets accepted and downsized to avatar_max_width/height; anything above
	// gets rejected outright so GD cannot be pushed into an OOM.
	const MAF_HARD_MAX_DIMENSION = 6000;
	const MAF_HARD_MAX_FILESIZE = 10485760; // 10 MB
	...
	*/
	public function prepare_form($request, $template, $user, $row, &$error)
	{
		...
		$template->assign_vars(array(
			...
			'AVATAR_UPLOAD_SIZE' => max((int) $this->config['avatar_filesize'], self::MAF_HARD_MAX_FILESIZE),
	...
	*/
	public function process_form($request, $template, $user, $row, &$error)
	{
		...
		$max_filesize = max((int) $this->config['avatar_filesize'], self::MAF_HARD_MAX_FILESIZE);
		...
		$resized = $this->maf_resize_if_needed($file, $error);
		if (count($error))
		{
			$file->remove();
			return false;
		...
		*/
		...
		return array(
			...
			'avatar_width' => $resized ? $resized['width'] : $file->get('width'),
			'avatar_height' => $resized ? $resized['height'] : $file->get('height'),
		);
	}
  ...
  protected function maf_resize_if_needed(\phpbb\files\filespec $file, array &$error)
	{
		...
		$max_w = (int) $this->config['avatar_max_width'];
		$max_h = (int) $this->config['avatar_max_height'];
		if ($max_w <= 0 || $max_h <= 0)
		{
			return null;
		}
		...
		$supported = get_supported_image_types($type);
		if (empty($supported['gd']) || $supported['format'] === false)
		{
		...
		$new_w = max(1, (int) floor($w * $ratio));
		$new_h = max(1, (int) floor($h * $ratio));

		if ($supported['format'] === IMG_GIF && $this->maf_resize_gif_with_imagick($source, $new_w, $new_h))
		{
		...
		if ($supported['format'] === IMG_PNG || $supported['format'] === IMG_GIF)
		{
		...
		switch ($supported['format'])
		{
			case IMG_JPG:  $ok = @imagejpeg($dst, $source, 90); break;
			case IMG_PNG:  $ok = @imagepng($dst, $source);      break;
			case IMG_GIF:  $ok = @imagegif($dst, $source);      break;
			case IMG_WEBP: $ok = function_exists('imagewebp') ? @imagewebp($dst, $source) : false; break;
		}
	...
	protected function maf_resize_gif_with_imagick($source, $new_w, $new_h)
	{
		...
		try
		{
			...
			// coalesceImages() renders each frame on the full canvas so per-frame
			// resize doesn't mangle GIFs that use delta / disposal frames.
			$imagick = $imagick->coalesceImages();
			foreach ($imagick as $frame)
			{
				$frame->thumbnailImage($new_w, $new_h);
				$frame->setImagePage($new_w, $new_h, 0, 0);
			}
			// deconstructImages() re-optimises back to delta frames so filesize
			// stays sane after the resize.
			$imagick = $imagick->deconstructImages();
			$ok = $imagick->writeImages($source, true);
			$imagick->clear();
			$imagick->destroy();
			return (bool) $ok;
		}
