	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			case 'pm_report_details':
				...
				while ($report_row = $db->sql_fetchrow($result))
				{
					$row_reason = array('title' => $report_row['reason_title'], 'description' => $report_row['reason_description']);
					if (isset($user->lang['report_reasons']['TITLE'][strtoupper($row_reason['title'])]) && isset($user->lang['report_reasons']['DESCRIPTION'][strtoupper($row_reason['title'])]))
					{
						$row_reason['description'] = $user->lang['report_reasons']['DESCRIPTION'][strtoupper($row_reason['title'])];
						$row_reason['title'] = $user->lang['report_reasons']['TITLE'][strtoupper($row_reason['title'])];
					}

					$template->assign_block_vars('reportrow', array(
						'REPORT_DATE'				=> $user->format_date($report_row['report_time']),
						'REPORT_REASON_TITLE'		=> $row_reason['title'],
						'REPORT_REASON_DESCRIPTION'	=> $row_reason['description'],
						'REPORT_TEXT'				=> make_clickable(bbcode_nl2br($report_row['report_text'])),
						'REPORTER_FULL'				=> get_username_string('full', $report_row['user_id'], $report_row['username'], $report_row['user_colour']),
