...
function close_report($report_id_list, $mode, $action, $pm = false)
{
	...
	global $phpEx, $phpbb_root_path, $phpbb_container, $table_prefix;
	...
	{
		$original_post_id_list = $post_id_list;
		$has_access = phpbb_check_ids($post_id_list, POSTS_TABLE, 'post_id', array('m_report'));

		// Re-admit posts that phpbb_check_ids removed but the current user has
		// PT access to via the privatetopics extension. Without this, PT-only
		// mods hit NOT_AUTHORISED on close/delete even when they can view the
		// report through mcp_get_post_data_add_pt_posts.
		$removed = array_values(array_diff($original_post_id_list, $post_id_list));
		if (!empty($removed))
		{
			$user_id = (int) $user->data['user_id'];
			$pt_sql = 'SELECT p.post_id
				FROM ' . POSTS_TABLE . ' p
				WHERE ' . $db->sql_in_set('p.post_id', $removed) . '
					AND (
						EXISTS (SELECT 1 FROM ' . $table_prefix . 'private_topic_users tu
							WHERE tu.topic_id = p.topic_id AND tu.user_id = ' . $user_id . ')
						OR EXISTS (SELECT 1 FROM ' . $table_prefix . 'topic_mod tm
							WHERE tm.topic_id = p.topic_id AND tm.user_id = ' . $user_id . ')
					)';
			$pt_result = $db->sql_query($pt_sql);
			while ($pt_row = $db->sql_fetchrow($pt_result))
			{
				$post_id_list[] = (int) $pt_row['post_id'];
				$has_access = true;
			}
			$db->sql_freeresult($pt_result);
		}
