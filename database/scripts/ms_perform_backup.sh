...
if [[ $MAFIASCUM_ENVIRONMENT != 'development' ]] && [[ $MAFIASCUM_ENVIRONMENT != 'dev' ]] && [[ $MAFIASCUM_ENVIRONMENT != 'local' ]]; then
    ...
    cd /tmp
    mysqldump -u${PHPBB_DATABASE_USER} -p${PHPBB_DATABASE_PASSWORD} --skip-lock-tables --quick --single-transaction --routines --triggers --databases ms_phpbb3 ms_mediawiki > ms_phpbb3_and_ms_mediawiki.sql
